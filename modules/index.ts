/**
 * AI CFO Suite - Module Registry
 * 
 * Central export point for all platform modules.
 */

// Active Modules
export * from './wip';

// Active Modules
export * from './discovery';

// Coming Soon Modules
export { FORECASTING_MODULE } from './forecasting';
export { CAPACITY_MODULE } from './capacity';
export { JCURVE_MODULE } from './jcurve';
export { BUDGET_MODULE } from './budget';
export { COVENANT_MODULE } from './covenant';
export { PROFITABILITY_MODULE } from './profitability';
export { BIDNOBID_MODULE } from './bidnobid';
export { SCENARIOS_MODULE } from './scenarios';
export { REPORTING_MODULE } from './reporting';

