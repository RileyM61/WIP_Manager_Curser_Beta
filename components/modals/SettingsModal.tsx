...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, settings, onSave }) => {
  ...
  const handleSave = (e: React.MouseEvent) => {
    ...
    console.log('Saving settings with logo:', settingsToSave.companyLogo ? 'Logo present' : 'No logo');
  ...
  const handleCropComplete = (croppedImage: string) => {
    setCurrentSettings(prev => {
      const updated = { ...prev, companyLogo: croppedImage };
      console.log('Logo cropped and set in currentSettings:', updated.companyLogo ? 'Logo present' : 'No logo');
      return updated;
    });
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition ${
                          currentSettings.capacityEnabled ? 'bg-brand-blue' : 'bg-gray-300 dark:bg-gray-600'
                        }`}
                      >
                        <span
                          className={`inline-block h-5 w-5 rounded-full bg-white transform transition ${
                            currentSettings.capacityEnabled ? 'translate-x-5' : 'translate-x-1'
                          }`}
                        />
                      </button>
                    </div>
